...
> **生成时间**: 2026-09-24 08:20（本轮=**全量重导的开闸前清障**，268 模型重导本身**待用户放行**）。① 查出并修好**三处"假绿灯"**（都在验收/换入侧而非导出侧，详见 `TROUBLESHOOTING.md` §24）：`apply_live2d_motions.py` 的 docstring 写 `L2D_OUT_DIR` 而代码读 `L2D_SRC_DIR`→照文档执行会**静默换入默认目录 `.diag/l2d_new` 里 09-21 修复前的陈旧产物**（已改为必须显式给源目录 + 曲线总数变少即拦）；`l2d_ref_diff.py` 在只含 `motion/` 的临时目录上**全部 SKIP 却退出 0**（已加 model3 回退 + 零比对即失败）；`l2d_ab.py` 的"新侧"目录硬编码同一份陈旧目录（已改跟 `L2D_OUT_DIR`）。② **样本重导已过闸**（`.diag/l2d_fix`，配方 `L2D_MOTION_LINEAR=1 L2D_MOTION_EMIT_CONST=1`）：`antu_2` 104/104 文件与已人工验收的生产数据**逐字节一致**；`aerbien_3` idle 曲线 445/640→**640/640**、关键帧不一致 195→**0**、偏差中位 **2.88→0.128**、>0.5 占比 **95.1%→40.8%**（同模型换入前判 FAIL，属严格改善；40.8% 仍超 12% 线，该模型 Unity 侧键稀疏、线性近似损失大）；`gaoxiong_7`/`lingbo` PASS 且偏差中位 0.0；`z46_3` 参考库无此模型。单模型 ~3.5s → 全量 269 约 20 分钟。③ **权威输入迁出 `.diag`**：`inputs/azdata/*.json`（9.2MB，sha256 台账 `MANIFEST.json` + 校验 `scripts/diag/check_inputs.py`，数据本体仍不入库），白名单/AGENTS.md/WF-15 已同步；`.diag/azdata_tree.json` 经核实是 14 字节 `Invalid input.` 报错残留、已从白名单剔除。④ **磁盘**：`.diag` 11G→**2.5G**（39 个无头 Chrome profile / 7.70GB，新工具 `scripts/diag/clean_diag_profiles.py` 三条硬判据后删除，已写进 WF-16 收尾）。⑤ **语音口径定了**：新增只读体检 `scripts/diag/l2d_voice_inventory.py`，实测 253 皮肤/2504 组/5914 ogg、**缺失 0 孤儿 0** → 结论是**语音产物无需单独备份**（可确定性再生），不可复原的是 `inputs/azdata`，口径落在输入侧（WF-17）。
