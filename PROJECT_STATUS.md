...
7. ⚠️ **阵营「晶环联盟」码待定——原方案前提已证伪，挂起待决策（2026-09-20）**：
   ...
   - 🔎 **2026-09-25 推进一半（见 §34）**：阵营名**不是表字段**，而是 `gametip` 里一段按数字 id 索引的短文案（`1200 东煌 / 1201 撒丁帝国 / 1202 北方联合 / 1205 自由鸢尾 / 1206 维希教廷 / 1207 鸢尾教国 / 1210 KizunaAI / 1211 hololive / 1213 偶像大师 / 1215 SSSS / 1218 META / 1219 闪乱神乐 / 1224 danmachi / **1226 晶环联盟**`）。但 `nationality` 码→这段 id **不是位置对应**（码 1=白鹰、3=重樱、97=`·META`、101=海王星…）；已收窄到 **5 个码无中文名**：`12`(只有"瓦尔帕莱索")、`99`(构建者、仲裁者·提尔瑞特·VII) **待用户按船名裁定**；`101`=海王星联动、`104`=KizunaAI、`117`=尼尔(A2/2B) 可按推断写死（辨认单见 §36）。`晶环联盟`=文案号 1226，最可能对应码 12，但这是推断、未替你写死。另 `world_port_data.port_camp` 反推这条**已否证**（同一个 camp 同时给出白鹰/皇家/北方联合）。原方案"从游戏配置解析阵营名表"仍不成立——**没有码→名的表**。
