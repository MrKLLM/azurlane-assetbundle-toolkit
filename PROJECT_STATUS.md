...
6. ✅→⏳ **声优中文姓名补全（答案表已到手，回填待放行；2026-09-25）**：用新解析出的 `voice_actor_cn` 已拿到 **525 条 `code → 中文声优`**（`.diag/sharecfg_re/voice_actor_cn.json`）；对 `ship_skin_template` 实际用到的 499 个 `voice_actor` 值，**只缺哨兵 `-1`/`0`，真实 id 覆盖率 100%** ⇒ "CV 姓名表未缓存"这个前提作废。**已接进 `build_ship_meta.py`（新增 `voice_actor_name`；只读诊断实测 库尔斯克→衣川里佳、泛用型布里→下田麻美）；`Output/ship_meta.json` 的 `--write` 重写待你放行**。见 §35。
7. ⚠️ **阵营「晶环联盟」码待定——原方案前提已证伪，挂起待决策（2026-09-20）**：
   ...
   - 🔎 **2026-09-25 推进一半（见 §34）**：阵营名**不是表字段**，而是 `gametip` 里一段按数字 id 索引的短文案（`1200 东煌 / 1201 撒丁帝国 / 1202 北方联合 / 1205 自由鸢尾 / 1206 维希教廷 / 1207 鸢尾教国 / 1210 KizunaAI / 1211 hololive / 1213 偶像大师 / 1215 SSSS / 1218 META / 1219 闪乱神乐 / 1224 danmachi / **1226 晶环联盟**`）。但 `nationality` 码→这段 id **不是位置对应**（码 1=白鹰、3=重樱、97=`·META`、101=海王星…）；已双证收窄到**只剩 2 个码待你定名：`12`(全盘只有"瓦尔帕莱索"一艘) 与 `99`(构建者、仲裁者·提尔瑞特·VII)**；`101`=海王星联动、`104`=KizunaAI、`117`=尼尔 已可写死。`晶环联盟`=文案号 1226，最可能对应码 12，但这是推断、未替你写死。另 `world_port_data.port_camp` 反推这条**已否证**（同一个 camp 同时给出白鹰/皇家/北方联合）。原方案"从游戏配置解析阵营名表"仍不成立——**没有码→名的表**。
