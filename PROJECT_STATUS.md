...
> ④ **权威基准 `l2d_ref_diff` 的判据已按 §26 重定义并实测**：旧判据给出 153 FAIL 而同一批数据文件级审计与浏览器回归全绿，根因是「关键帧不一致」把参考版 60fps 重采样算成硬错误 + 组→文件按 `files[0]` 盲配。现在 FAIL 只认真缺陷（漏组 / 配对成功下曲线缺失 / 偏差中位>0.5），并加了 12s 墙钟硬超时（原来 `urlopen(timeout=25)` 挡不住慢滴，实测挂死 25 分钟）、`.diag/refcache` 缓存、`[i/n]` flush 进度与每 10 个增量落盘、`--only` 分批。5 个已知样本上结论与改前一致（PASS 3/WARN 1/SKIP 1）但不再误报。**全库重跑（270 键 ≈1.3 万次请求，1~2 小时量级）进行中**。
> ⚠️ **唯一新开的问题（非重导引入）**：`hit_verify` 抽样 8 模型 **40/54 命中**，漏的全部是**嵌套框歧义**
>   （点 `touch_drag4` 播出 `touch_drag2`、`yingrui_3` Body→Special），系今日判定区从 3 个补到几十个的直接后果。
>   **建议先改前端竞选规则（嵌套时取「包含点且面积最小」的框）再重标定基线**，否则今天标完明天又要重标。
...
9. ✅ **Live2D 动作数据全量重导已落到 269 个模型（2026-09-24 完成；过程含一次覆写事故，见下）**
   ...
   **✅ 2026-09-24 全量完成**：`clips 8154 / shell 7（=真为空的 7 条，无新增失败）/ misassign 0 / PartOpacity 79 模型`、曲线 **944,136→2,662,615**、`fix_model3` 全库补登 `Touch*`、`l2d_sweep` 全量 **269 | 失败 0 | 未启动 0 | 未生效 0**。
   ⚠️ **本项已闭环但引出两件事**：① 判定区从 3 扩到几十个后 **`hit_verify` 基线必须重标定**（抽样 8 模型 40/54，漏的全是嵌套框歧义）——建议**先改前端竞选规则（取「包含点且面积最小」的框）再标定**；② 权威基准 `--all` 因参考库逐个 25s 超时仍在长尾运行，抽样结论已支持配方正确。
