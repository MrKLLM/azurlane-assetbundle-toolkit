...
⏳ **9 个 bundle 已重建到临时目录、待换入**：benningdun_2 / bunao_3 / feiteliekaer_4 / gangyishawa_3 / guanghui_9 / pulimaosi_3 / sebao_2 / shi_3 / wuzang_4（画廊里这些皮肤整块没有 Live2D）。产物在 `.diag/l2d_new9/`，逐模型审计 0 空壳 / 0 错位；换入清单见头部「换入待发」。
...
10. ✅ **Live2D 动作层根因重建（2026-09-21，用户报"乱飘/乱闪/没反应"）**：
   ...
   - ⚠️ **未完成的全量浏览器回归**：`scripts/diag/l2d_sweep.py` 已升级为内容判据（curveCount>0 且目标值变化），但它把 45~260 个模型塞进**一次** `Runtime.evaluate` 驱动，实测换数据后卡在第一条不返回（A/B 逐条驱动则正常）——已在脚本头写明「别把它当已通过的空壳检验」。全量浏览器回归请改成 Python 侧逐条 evaluate 后再跑；本轮结论依据是**全量文件级审计(8154 clip) + 换入前 4 模型浏览器 A/B 对照**。
...
11. ⏳ **HitAreas 生成规则漏了 `touch_*` 组名 → 「4 个模型无判定区」是误判（2026-09-21 查出，用户决定本轮不修）**：
   - **取证**：`chaijun_6/antu_2/baifeng_3/jinluhao_3` 与上述 9 个新模型的 moc3 里**都有** `TouchHead`/`TouchBody`/`TouchSpecial` 部件，动作组里也**都有** `touch_head`/`touch_body`/`touch_special`。老模型组名叫 `Head/Body/Special`，新皮肤叫 `touch_*`，而当初生成真 HitAreas 的那条路径只认前者（且它不在现役管线里，`fix_model3.py` 只会补占位 Id `HitArea/HitArea2`）。
   - **后果**：这 **13 个模型**点部位不能精确命中，走 §6.7 的兜底 → 「点哪儿都蹦一个 `touch_drag*`」，正是用户报过的行为；兜底范围也从来不是 4 个。
   - **修法（已定未实施）**：HitAreas = moc3 存在的 `Touch<X>` 部件 ↔ 该模型真实存在的动作组（按 `X`/`touch_x`/`tapx` 依次匹配，Name 用**实际组名**，前端 `index.html:458` 已支持小写回落）。改后须证：260 个既有 `model3.json` 除 `HitAreas` 外零字段变化、`hit_verify.py` 全库复跑 ≥767/768、这 13 个模型点头/身/特各自命中且点框外不播（兜底触发数应降为 0）。明细见 `docs/TROUBLESHOOTING.md` §18。
