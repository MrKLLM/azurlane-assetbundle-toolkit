...
> ④ **权威基准 `l2d_ref_diff --all` 在全库规模上判为"不可判"**（新 §26）：跑到 206/269 时日志 25 分钟零增长（连接半开，`urlopen(timeout=25)` 不触发），已手动停；已打印 **PASS 5 / WARN 22 / FAIL 153 / SKIP 26**，其中 FAIL 多数是**判据失真**（参考版 60fps 重采样 → "关键帧不一致"、组→文件按 `files[0]` 盲配 → 配对错）。同一批数据在文件级审计与浏览器回归下全绿 → **先怀疑判据不是先怀疑数据**；判据三条改法已写进 §26。抽样结论仍支持配方：`antu_2`/`gaoxiong_7`/`lingbo` PASS、`aerbien_3` FAIL→WARN（曲线 445/640→640/640、偏差中位 2.88→0.128）。
