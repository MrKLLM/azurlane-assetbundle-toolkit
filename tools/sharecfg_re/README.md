...
## 当前状态（2026-09-26 接管补记）：**文法已破、配置表可读；实质缺口只剩 21 个容器字段**

> 本节由后续会话接管补写。原会话把本 README 停在 09-24 的 (16)–(22)，而下面那条
> **「2026-09-23 收口：未破解，零明文字节到手」整节只作留痕，勿当现状读**（已就地标注）。
> 细节一律在 `docs/TROUBLESHOOTING.md` §30–§39 与 `docs/WORKFLOWS.md` WF-18/19/20，这里只留结论与判据。

| 结论 | 判据（可独立复算的量） | 详见 |
|---|---|---|
| `sharecfgdata/*` **不是加密**，是自定义容器：**假 LuaJIT 帧 + 平铺 kgc 常量流 + 字符串掩码 `c ^ (255-i)`**（i 每条串重新从 0 计） | ① **帧覆盖残差 32/32 全为 0**（记录长度之和 == 文件大小，空记录 0）；② `kgc` == 顶层常量条数且**末常量恰落在记录末**（把漂移定位到"第 k 条之后"）；③ 解出的字段集与 `azurlane-data` 人工 schema **逐项吻合** | §33 |
| 标量字段验收 **99.974%** | 值级交叉验证；差额逐条归因到基准 381 vs 本地 385 的版本漂移（能举出具体三条），**不用可打印率/熵当闸门** | §33 追加 |
| **台词中文可读**：2601 行 / 40,897 个中文字段值；32 张表含中文行 42,959 → **69,435（+62%）** | 导出层改前/改后**逐表对比零回退** | §33、`37_parse_sharecfgdata.py` |
| 磁盘只有 **32** 张流式表（其 `.lua.bytes` 是 reader stub），另 **740** 张 Lua-only 表数据**内联在同一容器** → 774 张全量导出 | `41_export_lua_tables.py`：DATA 639 / STUB 129 / 其它 4，**未走完 0** | §35 |
| 两张权威表已进产品管线：声优 525 条、台词↔动作组↔音频资源 85 条 | `42_publish_gamecfg.py` 真值自检 + E2 全量重导 **+759 动作组**、浏览器实播四条通过 | §35/§36/§38、WF-17 追加 |
| `www()` 已完整还原：19×int32 内联密钥 + Phase C 四种 XXTEA/XTEA 变体按块号 `%4` 循环；`scripts64` 整包解出、UnityPy 可开 | 判据 = 解出头 8 字节 `UnityFS\0`（全库 91,642 个 AB 里只有这 2 个是密文头） | §30–§32、WF-19 |
| `{namecode:NNN}` ↔ `name_code.id` **已证成**（需要真名可直接解占位符） | 自称率 2.13% vs 200 次随机重分配零假设 0.03%（**80×**）；1760/1760 码可查 | §36、`43_check_namecode_alignment.py` |

**唯一实质缺口**：**21 个容器字段**（`smoke` / `bound_bone` / `l2d_se` / `fx_container` / `time` / `get_showing` 等，占字段对 10.3%）的值**不内联**，
由假 BC 指令流（4 字节/条，第 2 字节 `ff→fe→fd→fc` = 栈回溯距离）在运行时拼装；第三方实现同样没解（其注释：
"SchemaReader is not gonna work with op codes"）。所以**"表读通了" ≠ "字段齐了"**，报进展时这两件事必须分开说。

**「下一步（第 6 次收口）」四项已全部有答案，不再挂账**：
1. ~~19 字节数组到底参与什么~~ → 它是 `www()` 的 **19×int32 内联密钥**，从 `global-metadata.dat` 的
   `fieldDefaultValues` 堆**按内容**取出（不需要 token→索引映射；该堆的节序是 v31 特化的，别按 v24–29 记）。见 WF-19。
2. ~~主循环边界 / trailer 语义 / 种子还叠了什么~~ → **trailer 是 BE32**；长度 L 用 AB 自报 fileSize 与
   `filesize-4-L` 互校钉死（两个文件各自命中）；密钥与四模式已逐指令落成，无需再猜种子。
3. ~~www 的输入是哪一段~~ → `PathUtil.GetLuaBundle()` → `ReadAllBytes()`，输入就是盘上的 **`scripts64`/`scripts32` AB 本体**；
   设备 BSA 分支本机不走，无需解 BSA 归档。
4. ~~用反解器验一次~~ → **已命中**（判据 `UnityFS` 正确且可达，(16) 顶部那条更正成立）。

### 下一步（第 7 次收口，2026-09-26）

1. **指令流解码**（唯一能让那 21 个字段成形的路）：拿 `ship_skin_template` 里一个已知容器字段（如 `smoke`）当靶子，
   把该记录 `insn` 区按 4 字节切片，先按"第 2 字节 = 栈回溯距离"假设回放。
   **判据**：回放结果 == `azdata` 基准里同一行的真实值。**先单表单字段打通，再谈全表**；不成即判死并写进「已排除的假设」。
2. **兜底方案**：若 1 判死，改为按表手写 schema 字段清单（参照实现的做法），把这 21 个字段显式标 `assembled-by-code`。
   ⚠️ 不得再产出"结构合法的占位空值"冒充成功（那条静默失败已记进技能 `safe-pipeline-fix-targeted-rerun`）。
3. 两项展示层待用户裁定：`build_gallery_index` 是否改成"配置名优先于手抄表"（会顺带改 **112 组**显示名）；
   `ship_meta` 剩余 **52 条** `{namecode}` 占位符是否正式解进 `cn`。
4. 版本更新时按 **WF-15** 白名单跑双源对拍：`inputs/azdata`（社区快照，会滞后）+ `inputs/gamecfg`
   （从游戏本体解出，**已比快照新**，冲突时以它为准），闸门用 `scripts/diag/ship_meta_authority_diff.py`。

---

## 当前状态（2026-09-23 收口，**仅作留痕，勿当现状**）：当时未破解
...
> **09-26 更正**：前半句已不成立（明文与台词中文均已解出，见上面接管补记）；后半句"已知明文自证判据"这条**方法论仍然成立**，
> 且正是最后收口用的那道验收（值级交叉验证 99.974%）。
...
> ⚠️ 本节是 09-23 写的，第 1–5 步**已全部完成**（台词明文已到手，见上面「2026-09-26 接管补记」）；第 1–3 步（metadata 解析、version=31 判定、Il2CppDumper + `08_disasm_method.py` 逐方法反汇编）当时即已完成。当前有效的待办见上面「下一步（第 7 次收口）」。
