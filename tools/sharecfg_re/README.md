...
## 当前状态（2026-09-26 接管补记）：**文法已破、配置表可读；剩余缺口 = 21 个字段在现行读法下取不到值（成因未证）**
...
**唯一实质缺口（事实与假设分两层写）**
- **实测事实**：**21 个字段**（`smoke` / `bound_bone` / `l2d_se` / `fx_container` / `time` / `get_showing` 等，占字段对 10.3%）
  在现行读法下**取不到值**——它们不像标量那样以 `tag + 载荷` 内联出现。这条是"标量字段验收 99.974%"里那 0.026%
  与逐表字段清单对拍出来的，与解释无关。
- **未证假设（别当已知事实用）**：我给出的解释是"其值由假 BC 指令流在运行时拼装"。
  它**只解释了"为什么内联读不到"，没有被正向验证**（没有回放任何一条指令并还原出基准值）。
  ⚠️ 另一会话正按"`byte-array n` 标签被误当容器"的方向复核（含中文行 69,435 → 116,024 量级）——
  **若那条路成立，本段的"指令流拼装"解释即为错，以它为准**；截至本条补记写就时，其结论尚未落进本 README 或 §30–§39。
- **可迁移的教训**：某一类字段整体读不出时，**先怀疑自己的标签/类型解释，再怀疑"运行时才拼装"**。
  后者的证明成本要高得多，而前者只要改一行读法就能重跑全表对拍。

所以**"表读通了" ≠ "字段齐了"**，报进展时这两件事必须分开说。
...
1. **先花十分钟排除"标签解释错"**（成本最低，且是另一会话正在复用的方向）：把这 21 个字段的起始字节当作
   "可能是一个我之前没实现的标量/字节数组标签"，改一行读法**重跑全表**，判据仍是
   "该字段值 == `azdata` 基准里同一行的真实值"。命中即整条"运行时拼装"假设作废。
2. **再谈指令流解码**（只有 1 判死才值得做）：把该记录 `insn` 区按 4 字节切片回放。
   ⚠️ 本项目在 (15)/(28) 两次因为**固定长度反汇编窗口越过函数尾**、以及**把结构里的字节三连当 magic**
   而得出过头结论——指令流方向必须先有"能回放出一条已知值"的正向证据，再谈整表。
   **先单表单字段打通，再谈全表**；不成即判死并写进「已排除的假设」。
3. **兜底方案**：若 1、2 都判死，改为按表手写 schema 字段清单（参照实现的做法），把这 21 个字段显式标 `assembled-by-code`。
   ⚠️ 不得再产出"结构合法的占位空值"冒充成功（那条静默失败已记进技能 `safe-pipeline-fix-targeted-rerun`）。
4. 两项展示层待用户裁定：`build_gallery_index` 是否改成"配置名优先于手抄表"（会顺带改 **112 组**显示名）；
   `ship_meta` 剩余 **52 条** `{namecode}` 占位符是否正式解进 `cn`。
5. 版本更新时按 **WF-15** 白名单跑双源对拍：`inputs/azdata`（社区快照，会滞后）+ `inputs/gamecfg`
