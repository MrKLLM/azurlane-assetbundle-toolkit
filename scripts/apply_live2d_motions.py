...
用法:
  ...
  py -3 scripts/apply_live2d_motions.py --yes                 # 换入
  L2D_OUT_DIR=<源目录> py -3 scripts/apply_live2d_motions.py --yes
"""
...
SRC = os.environ.get("L2D_SRC_DIR") or os.path.join(ROOT, ".diag", "l2d_new")
...
def main():
    ...
    if args.yes:
        print(f"[+] 已换入 {len(models)} 个模型的 motion/，旧数据备份在 {backup}")
        print("[i] 下一步: 1) py -3 scripts/fix_model3.py   2) 画廊回归 + 增量重建")
    else:
        print("[i] 干跑结束；确认无误后加 --yes 执行")
    return 0
