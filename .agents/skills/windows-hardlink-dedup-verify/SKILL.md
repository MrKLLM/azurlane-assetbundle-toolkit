...
5. **配一个长期漂移探测器**：把 copy 模式降级为"回退手段"，另加一个只读 `--check`（比对 inode/哈希，漂移即非零退出）。因为**硬链唯一会被悄悄打断的场景是编辑器用「写临时文件 + rename」保存**——那一刻起两边又成独立副本且开始各自演化，没有探测器的话，事故会在很久以后以"我的改动怎么没了"的形式回来。
